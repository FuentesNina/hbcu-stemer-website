...
export default function NewsletterForm({newsletterForm}: {newsletterForm: any}) {
  ...
  return (
    <form ref={formRef} action={`https://formsubmit.co/${newsletterForm.email}`} method="POST" className="relative top-10 bg-myRed p-10 md:grid md:grid-cols-[3fr_2fr] md:gap-5 md:max-w-3xl md:mx-auto md:rounded-2xl lg:max-w-5xl"
            >
        ...
        <div className="md:my-auto">
            <Title content={newsletterForm.title} className="text-white text-center " />
            <p className="font-body text-white text-center mt-5 mb-10 max-w-xs mx-auto md:mb-0 md:max-w-md">{newsletterForm.description}</p>
        </div>

        <div className="flex flex-col max-w-sm mx-auto md:w-full">
            {newsletterForm.formFields?.map((field: any, index: number) => {
            ...
            <Button href="" buttonStyle="white-black-on-red" content={newsletterForm.button} type="submit" className="my-5 md:mb-0"/>
        </div>
    </form>
  )
}
