    ...
    return (
      <>
        ...
        <nav onClick={closeMenu} className={`${mobileMenu ? 'w-screen h-screen bg-black/[0.25] backdrop-blur-sm' : "w-0 h-0"}  overflow-hidden md:hidden z-50 fixed`} >
          <div className={`${mobileMenu ? 'left-full animate-showMenu' : (firstLoad ? "hidden" : 'animate-hideMenu right-0')} p-14 pt-5 min-w-max md:hidden bg-black/[0.9] rounded-bl-xl backdrop-blur fixed`}>
